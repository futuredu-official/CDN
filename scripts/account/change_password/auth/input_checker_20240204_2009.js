function input_checker() {
	...
	if (txt_password.length < 8) {
		...
		document.getElementById("desc_password_msg").className = class_desc_msg
		ReactDOM.render(<Div_desc_err_msg text="'비밀번호 확인'은 8자 이상이어야 합니다."/>, document.getElementById("desc_password_msg"))

	} else if (txt_password != txt_password_confirm) {
		document.getElementById("desc_password_msg").className = class_desc_msg
		ReactDOM.render(<Div_desc_err_msg text="비밀번호와 비밀번호 확인이 일치하지 않습니다."/>, document.getElementById("desc_password_msg"))
